...
# ## Step 2 — Load Delete Signals
# 
# Read delete flags from integration tables to identify which EngagementIDs and MonthlyDataIDs should be removed.
# 
# These signals will be used to:
# 1. **Delete existing rows from Bronze** (Step 3)
# 2. **Filter new Raw data** during transformation (Step 4)
...
if len(target_dates) == 0:
    ...
    print(f"EngagementIDs to delete : {eng_delete_count:,}")
...
if len(target_dates) == 0:
    ...
    print(f"MonthlyDataIDs to delete: {monthly_delete_count:,}")
...
# ## Step 3 — Delete Rows from Bronze (Target Table)
# 
# **Important**: Delete rows from the Bronze table based on delete signals **before** processing new data.
# 
# This ensures:
# - Old rows with deleted EngagementIDs or MonthlyDataIDs are removed from Bronze
# - The target table stays clean even if delete signals arrived after data was already written
# - Idempotent: safe to run multiple times
# 
# Uses Delta table DELETE operations which leverage file-level statistics for efficient pruning.

# CELL ********************

# Delete rows from Bronze table based on delete signals
print("Deleting flagged rows from Bronze table...")

# Check if Bronze table exists before attempting delete
if not spark.catalog.tableExists(BRONZE_WORK_UNITS):
    print("⚠ Bronze table does not exist yet — skipping delete operation")
else:
    bronze_delta = DeltaTable.forName(spark, BRONZE_WORK_UNITS)
    
    # Strategy: Use MERGE with WHEN MATCHED THEN DELETE
    # This is more efficient than subqueries and supported by Delta
    
    deleted_total = 0
    
    # Delete by EngagementID using MERGE
    if eng_delete_count > 0:
        print(f"  Deleting rows matching {eng_delete_count:,} EngagementIDs...")
        
        bronze_delta.alias("target").merge(
            df_eng_deletes.alias("source"),
            "target.EngagementID = source.EngagementID"
        ).whenMatchedDelete().execute()
        
        print(f"    ✓ EngagementID delete complete")
    
    # Delete by MonthlyDataID using MERGE
    if monthly_delete_count > 0:
        print(f"  Deleting rows matching {monthly_delete_count:,} MonthlyDataIDs...")
        
        # Refresh Delta handle after first delete
        bronze_delta = DeltaTable.forName(spark, BRONZE_WORK_UNITS)
        
        bronze_delta.alias("target").merge(
            df_monthly_deletes.alias("source"),
            "target.MonthlyDataID = source.MonthlyDataID"
        ).whenMatchedDelete().execute()
        
        print(f"    ✓ MonthlyDataID delete complete")
    
    if eng_delete_count == 0 and monthly_delete_count == 0:
        print(f"  No deletes to apply")
    else:
        # Show delete metrics from Delta history (reads metadata only)
        print(f"\n  ✓ Delete operations complete")
        try:
            history = spark.sql(f"DESCRIBE HISTORY {BRONZE_WORK_UNITS}").limit(2).collect()
            total_deleted = 0
            for h in history:
                if h.operation == "MERGE":
                    metrics = h.operationMetrics
